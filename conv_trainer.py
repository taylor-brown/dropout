...
from convolutional_mlp import LeNetConvPool3dLayer, relu

from logistic_sgd import LogisticRegression
from mlp import HiddenLayer, _dropout_from_layer, DropoutHiddenLayer, MLP
...
def evaluate_lenet5(build_model, learning_rate=.1, n_epochs=200, batch_size=128):
    ...
    # BUILD ACTUAL MODEL #
    ######################
    cost, last_layer, updates = build_model(batch_size, learning_rate, rng, x, y)
    ...
    test_model = theano.function([index], last_layer.errors(y),
                                 givens={
                                     x: test_set_x[index * batch_size: (index + 1) * batch_size],
                                     y: test_set_y[index * batch_size: (index + 1) * batch_size]})

    validate_model = theano.function([index], last_layer.pred_prob(),
                                     givens={
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                probs = [validate_model(i) for i
                         in xrange(n_valid_batches)]
                fpr, tpr, _ = metrics.roc_curve(data_y[4400:4656], numpy.concatenate(probs)[:, 1])
                this_validation_loss = 1 - metrics.auc(fpr, tpr)
...
def experiment(state, channel):
    evaluate_lenet5(build_model_example, state.learning_rate)
